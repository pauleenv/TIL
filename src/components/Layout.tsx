  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="bg-transparent p-4 flex justify-between items-center"> {/* Removed bg-primary, text-primary-foreground */}
        <img src="/Logo-1.svg" alt="AJA Logo" className="h-12 w-auto" /> {/* Replaced h1 with logo */}
      ...
      <nav className="fixed bottom-5 left-4 right-3 bg-white border-2 border-black shadow-custom-black-lg rounded-[16px] z-50"> {/* Added border-2 border-black */}
          ...
          <TabsList className="grid w-full grid-cols-3 h-16 bg-transparent"> {/* Changed to 3 columns, removed bg-primary */}
            <TabsTrigger value="home" asChild>
              <Link to="/" className={cn(
                ...
                currentPath === "/" && "bg-white shadow-custom-black rounded-[16px] mx-2 my-1.5" // Active tab style: removed border-2 border-black
              )}>
            ...
            <TabsTrigger value="encyclopedia" asChild>
              <Link to="/encyclopedia" className={cn(
                "flex flex-col items-center justify-center text-xs sm:text-sm text-black", // Text color black
                currentPath === "/encyclopedia" && "bg-white shadow-custom-black rounded-[16px] mx-2 my-1.5" // Active tab style: removed border-2 border-black
              )}>
